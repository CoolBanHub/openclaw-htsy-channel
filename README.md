...
OpenClaw 的 HTSY Open Channel 插件，用于对接 HTSY Open API（微信消息回调与发送）。
...
npm install @coolban/openclaw-htsy-channel
```
...
channels:
  htsy_open:
    enabled: true
    # HTSY Open API 端点
    endpoint: "https://htsy.cpshelp.cn"
    # 应用 ID
    appId: "your-app-id"
    # 应用密钥
...
| `endpoint` | string | 否 | https://htsy.cpshelp.cn | HTSY Open API 端点 |
